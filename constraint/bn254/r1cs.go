...
type VariableInjector func([]fr.Element) (fr.Element, error)
...
func (cs *R1CS) Solve(witness, a, b, c fr.Vector, injectors map[string]VariableInjector, opt backend.ProverConfig) (fr.Vector, error) {
	...
	solution, err := newSolution(nbWires, opt.HintFunctions, cs.MHintsDependencies, cs.MHints, cs.Coefficients, &cs.System.SymbolTable, injectors)
	if err != nil {
	...
	if len(witness) != len(cs.Public)-1+len(cs.Secret) { // - 1 for ONE_WIRE
		err = fmt.Errorf("invalid witness size, got %d, expected %d = %d (public) + %d (secret)", len(witness), len(cs.Public)-1+len(cs.Secret), len(cs.Public)-1, len(cs.Secret))
		log.Err(err).Send()
...
func (cs *R1CS) IsSolved(witness witness.Witness, opts ...backend.ProverOption) error {
	...
	_, err = cs.Solve(v, a, b, c, nil, opt)
	return err
}
...
func (cs *R1CS) solveConstraint(r constraint.R1C, solution *solution, a, b, c *fr.Element) error {
	...
	processLExp := func(l constraint.LinearExpression, val *fr.Element, locValue uint8) error {
		for _, t := range l {
			...
			if attempted, err := solution.attemptSolveWithFunction(vID); attempted {
				if err != nil {
					return err
				}
