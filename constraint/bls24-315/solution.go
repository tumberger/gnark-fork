...
type solution struct {
	...
	mInjected            map[int]constraint.InjectedVariable
	injectors            map[string]VariableInjector
	st                   *debug.SymbolTable
}

func newSolution(nbWires int, hintFunctions map[hint.ID]hint.Function, hintsDependencies map[hint.ID]string, mHints map[int]*constraint.Hint, coefficients []fr.Element, st *debug.SymbolTable, injectors map[string]VariableInjector) (solution, error) {
...
// solveWithFunction unifies solveWithHint and solveWithInjector
// if it is not a hint/injection solvable variable the result will be false
func (s *solution) attemptSolveWithFunction(vID int) (bool, error) {
	if hnt, ok := s.mHints[vID]; ok {
		if err := s.solveWithHint(vID, hnt); err != nil {
			return true, err
		}
	}
	if v, ok := s.mInjected[vID]; ok {
		if err := s.solveWithInjector(v); err != nil {
			return true, err
		}
	}
...
func (s *solution) solveWithHint(vID int, h *constraint.Hint) error {
	...
	recursiveSolve := func(t constraint.Term) error {
		...
		// unsolved dependency
		if attempted, err := s.attemptSolveWithFunction(vID); attempted {
			return err
		} else {
...
// solveWithInjector compute solution.values[vID] using provided variable injector
func (s *solution) solveWithInjector(v constraint.InjectedVariable) error {
	// skip if the wire is already solved by a call to the same hint
	// function on the same inputs
	if s.solved[v.Wire] {
		return nil
	...
	// for each input, we set its value, IF all the wires are solved
	// the only case where all wires may not be solved, is if one of the input of this injector
	// is the output of a hint or another injector.
	// it is safe to recursively solve this with the parallel solver, since all hints-output wires
	// that we can solve this way are marked to be solved with the current constraint we are processing.
	recursiveSolve := func(t constraint.Term) error {
		if t.IsConstant() {
		...
		// unsolved dependency
		if attempted, err := s.attemptSolveWithFunction(v.Wire); attempted {
			return err
		}
		// it's not a hint, we panic.
		panic("solver can't compute hint; one or more input wires are unsolved")
	...
	inputs := make([]fr.Element, len(v.Inputs))
	for i := range inputs {
		for _, term := range v.Inputs[i] {
			if err := recursiveSolve(term); err != nil {
				return err
			}
			s.accumulateInto(term, &inputs[i])
		}
	}
