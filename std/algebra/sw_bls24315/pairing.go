...
func FinalExponentiation(api frontend.API, e1 fields_bls24315.E24) fields_bls24315.E24 {
	...
	var _t [3]fields_bls24315.E24
	_result := result.DecompressT2(api)
	_t[0].CyclotomicSquare(api, _result)
	...
	_t[2].Mul(api, _t[0], _t[2])
	_t[1].Expt(api, _t[2], genT)
	_t[1].Expt(api, _t[1], genT)
	_t[1].Expt(api, _t[1], genT)
	_t[1].Expt(api, _t[1], genT)
	_t[0].FrobeniusQuad(api, _t[2])
	_t[0].Mul(api, _t[0], _t[1])
	_t[2].Conjugate(api, _t[2])
	_t[0].Mul(api, _t[0], _t[2])
	_result.Mul(api, _result, _t[0])
