...
double _geoAzimuthRads(const LatLng *p1, const LatLng *p2) {
    // INSERTED BY LUCA
    /*printf("%f %f Arg1: %f  Arg2.1: %f Arg2.2: %f Arg2: %f res:%f \n", p2->lng, p1->lng, cos(p2->lat) * sin(p2->lng - p1->lng),
    		...
    		cos(p1->lat) * sin(p2->lat) - sin(p1->lat) * cos(p2->lat) * cos(p2->lng - p1->lng),
    						cos(p2->lng - p1->lng));
    */						
    double tmp = (cos(p2->lat) * sin(p2->lng - p1->lng)) / (cos(p1->lat) * sin(p2->lat) -
                     sin(p1->lat) * cos(p2->lat) * cos(p2->lng - p1->lng));
                     
    //printf("tmp: %f atan(tmp): %f atan2 result: %f\n", tmp, atan(tmp),
    //					atan2(cos(p2->lat) * sin(p2->lng - p1->lng),
    //             cos(p1->lat) * sin(p2->lat) - sin(p1->lat) * cos(p2->lat) * cos(p2->lng - p1->lng)));
    
    printf("atan2 result: %f\n", atan2(cos(p2->lat) * sin(p2->lng - p1->lng),
                 cos(p1->lat) * sin(p2->lat) - sin(p1->lat) * cos(p2->lat) * cos(p2->lng - p1->lng)));
