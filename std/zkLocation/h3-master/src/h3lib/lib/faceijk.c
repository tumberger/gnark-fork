...
// void _geoToFaceIjk(LatLng *g, int res, FaceIJK *h) { // const LatLng removed by Luca
    
//     // INSERTED BY LUCA
//     double pi = M_PI;
//     //printf("Lat & Lng: %f, %f", g->lat, g->lng);
//     //for(res = 0; res < 16; res++)
//     //{
//     //    printf("\n\n// Resolution %d\n", res); 
//     for(double i = -1.5; i <= 1.5; i += 0.5)
//     {
//     for(double j = -3.1; j < pi; j += 0.5)
//     {
    
//     //double i = -0.5; double j = -1.6; 
//     //int negativeLat = 0;
//     //int negativeLng = 0;
//     //if(i<0)
//     //    negativeLat = 1;
...
//     g->lat = i; g->lng = j;
//     printf("\n\t\t------------------\n\nLat & Lng: %f, %f\n", g->lat, g->lng);
//    // printf("assert.ProverSucceeded(&ijkCircuit, &Circuit{Lat: %0.9f, Lng: %0.9f,\nLatIsNegative: %d, LngIsNegative: %d, ", fabs(g->lat), fabs(g->lng), negativeLat, negativeLng);
...
// void _geoToHex2d(LatLng *g, int res, int *face, Vec2d *v) { // const LatLng removed by Luca
//     // determine the icosahedron face
//     double sqd;
//     _geoToClosestFace(g, face, &sqd);
...
//     // INSERTED BY LUCA
//     // printf("AzRad = %.9f, adjusted: %f\n", faceAxesAzRadsCII[*face][0], _posAngleRads(_geoAzimuthRads(&faceCenterGeo[*face], g)));
                      
//     /*for (int f = 0; f < NUM_ICOSA_FACES; ++f) {
//         printf("cosFace%dLat:= %.9f  sinFace%dLat:= %.9f\n", f, cos(faceCenterGeo[f].lat), f, 										sin(faceCenterGeo[f].lat));
...
//     // INSERTED BY LUCA
//     double nominator = sqrt( (-1) * (sqd-4) * sqd );
//     double alt = sqrt( (-1) * (sqd-4) * sqd ) / (2 - sqd);
//     // printf("\ntheta: %f  -- r: %f and alternative: %f  -- nominator: %f\n", theta, r, alt, nominator);

//     // scale for current resolution length u
//     r /= RES0_U_GNOMONIC;
    
//     // INSERTED BY LUCA
//     // printf("r after gnomic: %f\n", r);
...
//     // INSERTED BY LUCA
//     // printf("r after loop: %f\nsin(theta): %f --> y: %f\ncos(theta): %f --> x: %f\n", r, sin(theta), v->y, cos(theta), v->x);
// }
...
void _geoToClosestFace(const LatLng *g, int *face, double *sqd) {
    ...
    *sqd = 5.0;
    for (int f = 0; f < NUM_ICOSA_FACES; ++f) {
        ...
        // INSERTED BY LUCA
        //printf("Square distance for face %d: %f\n", f, sqdT);
    }
}
