...
func proverInjectors(r1cs *cs.R1CS, pk *ProvingKey, proof *Proof) map[string]cs.VariableInjector {
	return map[string]cs.VariableInjector{
		"bsb22": commitmentProverInjector(r1cs, pk, proof),
	}
}
...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, witness fr.Vector, opt backend.ProverConfig) (*Proof, error) {
	...
	proof := &Proof{}
	injectors := proverInjectors(r1cs, pk, proof)

	var wireValues []fr.Element
	var err error
	if wireValues, err = r1cs.Solve(witness, a, b, c, injectors, opt); err != nil {
		if !opt.Force {
			return nil, err
		} else {
...
func computeH(a, b, c []fr.Element, domain *fft.Domain) []fr.Element {
	...
	// reusing a to avoid unnecessary memalloc
