...
type solution[E any, ptE field.Element[E]] struct {
	values, coefficients []E
	solved               []bool
	nbSolved             uint64
...
func newSolution[E any, ptE field.Element[E]](nbWires int, hintFunctions map[hint.ID]hint.Function, hintsDependencies map[hint.ID]string, mHints map[int]*compiled.Hint, coefficients []E) (solution[E, ptE], error) {

	s := solution[E, ptE]{
		values:          make([]E, nbWires),
...
func (s *solution[E, ptE]) set(id int, value E) {
	if s.solved[id] {
...
func (s *solution[E, ptE]) isValid() bool {
	return int(s.nbSolved) == len(s.values)
}

// computeTerm computes coef*variable
func (s *solution[E, ptE]) computeTerm(t compiled.Term) E {
	cID, vID, _ := t.Unpack()
	...
	case compiled.CoeffIdTwo:
		var res E
		ptE(&res).Double(&s.values[vID])
		return res
	case compiled.CoeffIdMinusOne:
		var res E
		ptE(&res).Neg(&s.values[vID])
		return res
	default:
		var res E
		ptE(&res).Mul(&s.coefficients[cID], &s.values[vID])
		return res
	}
}

// r += (t.coeff*t.value)
func (s *solution[E, ptE]) accumulateInto(t compiled.Term, r, res *E) {
	cID := t.CoeffID()
	vID := t.WireID()
	switch cID {
	case compiled.CoeffIdZero:
		return
	case compiled.CoeffIdOne:
		ptE(r).Add(r, &s.values[vID])
	case compiled.CoeffIdTwo:
		ptE(res).Double(&s.values[vID])
		ptE(r).Add(r, res)
	case compiled.CoeffIdMinusOne:
		ptE(r).Sub(r, &s.values[vID])
	default:
		ptE(res).Mul(&s.coefficients[cID], &s.values[vID])
		ptE(r).Add(r, res)
	}
...
func (s *solution[E, ptE]) solveWithHint(vID int, h *compiled.Hint) error {
	...
	for i := 0; i < nbInputs; i++ {
		...
		case compiled.LinearExpression:
			...
			for _, term := range t {
				...
				s.accumulateInto(term, &v, &scratch)
			}
			ptE(&v).ToBigIntRegular(inputs[i])
		case compiled.Term:
...
func (s *solution[E, ptE]) printLogs(log zerolog.Logger, logs []compiled.LogEntry) {
...
func (s *solution[E, ptE]) logValue(log compiled.LogEntry) string {
	...
	for j := 0; j < len(log.ToResolve); j++ {
		if log.ToResolve[j] == compiled.TermDelimitor {
			...
			if missingValue {
				...
				toResolve = append(toResolve, ptE(&eval).String())
			}
			continue
		}
		...
		if isEval {
			...
			if visibility == schema.Virtual {
				...
				ptE(&eval).Add(&eval, &s.coefficients[cID])
				continue
			}
			if !s.solved[vID] {
				missingValue = true
		...
		if visibility == schema.Virtual {
			...
			if cID == compiled.CoeffIdMinusOne {
				...
				toResolve = append(toResolve, ptE(&s.coefficients[cID]).String())
			}
			continue
		}
		...
		if !s.solved[vID] {
			...
			toResolve = append(toResolve, ptE(&s.values[vID]).String())
		}
	}
