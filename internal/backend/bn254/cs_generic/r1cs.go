...
type R1CS[E any, ptE field.Element[E]] struct {
	compiled.R1CS
	Coefficients []E // R1C coefficients indexes point here
}

// NewR1CS returns a new R1CS and sets cs.Coefficient (E) from provided big.Int values
func NewR1CS[E any, ptE field.Element[E]] (cs compiled.R1CS, coefficients []big.Int) *R1CS[E, ptE] {
	r := R1CS[E, ptE]{
		R1CS:         cs,
		Coefficients: make([]E, len(coefficients)),
	}
	for i := 0; i < len(coefficients); i++ {
		ptE(&r.Coefficients[i]).SetInterface(&coefficients[i])
	}
...
func (cs *R1CS[E, ptE]) Solve(witness, a, b, c []E, opt backend.ProverConfig) ([]E, error) {
	...
	solution, err := newSolution[E, ptE](nbWires, opt.HintFunctions, cs.MHintsDependencies, cs.MHints, cs.Coefficients)
	if err != nil {
		return make([]E, nbWires), err
	}
	start := time.Now()
...
func (cs *R1CS[E, ptE]) parallelSolve(a, b, c []E, solution *solution[E, ptE]) error {
	...
	for i := 0; i < runtime.NumCPU(); i++ {
		go func() {
			var wire, check, scratch E 
			for t := range chTasks {
				for _, i := range t {
					// for each constraint in the task, solve it.
					if err := cs.solveConstraint(cs.Constraints[i], solution, &a[i], &b[i], &c[i], &wire, &check, &scratch); err != nil {
...
func (cs *R1CS[E, ptE]) IsSolved(witness *witness.Witness, opts ...backend.ProverOption) error {
	opt, err := backend.NewProverConfig(opts...)
	if err != nil {
		return err
	}

	a := make([]E, len(cs.Constraints))
	b := make([]E, len(cs.Constraints))
	c := make([]E, len(cs.Constraints))
	v := witness.Vector.(*bn254witness.Witness)

	vv := make([]E, len(*v))
	for j, i := range *v {
		ptE(&vv[j]).SetInterface(&i)
	}
	_, err = cs.Solve(vv, a, b, c, opt)
	return err
}

// divByCoeff sets res = res / t.Coeff
func (cs *R1CS[E, ptE]) divByCoeff(res *E, t compiled.Term) {
	cID := t.CoeffID()
	switch cID {
	...
	default:
		...
		ptE(res).Div(res, &cs.Coefficients[cID])
	}
}
...
func (cs *R1CS[E, ptE]) solveConstraint(r compiled.R1C, solution *solution[E, ptE], a, b, c, wire, check, scratch *E) error {
	...
	processLExp := func(l compiled.LinearExpression, val *E, locValue uint8) error {
		for _, t := range l {
			vID := t.WireID()

			// wire is already computed, we just accumulate in val
			if solution.solved[vID] {
				solution.accumulateInto(t, val, scratch)
				continue
			}
	...
	if loc == 0 {
		...
		// var check E
		ptE(check).Mul(a, b)
		if !ptE(check).Equal(c) {
			return fmt.Errorf("%s ⋅ %s != %s", ptE(a).String(), ptE(b).String(), ptE(c).String())
		}
		return nil
	}
...
func (cs *R1CS[E, ptE]) GetConstraints() [][]string {
...
func (cs *R1CS[E, ptE]) vtoString(l compiled.LinearExpression) string {
...
func (cs *R1CS[E, ptE]) termToString(t compiled.Term, sbb *strings.Builder) {
	tID := t.CoeffID()
	if tID == compiled.CoeffIdOne {
		...
		sbb.WriteString(ptE(&cs.Coefficients[tID]).String())
		sbb.WriteString("⋅")
	}
	vID := t.WireID()
...
func (cs *R1CS[E, ptE]) ReadFrom(r io.Reader) (int64, error) {
